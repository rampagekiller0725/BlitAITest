...
const HFieldGraph = (props: HFieldGraphProps) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (isSimulationComplete && !dataFetched) {
        const fetchedData = await getHField(
          projectId,
          version,
          username,
          setProgress
  ...
  if (visible && dataFetched) {
    ...
    return <div>Loading H-Field ({progress.toFixed(0)}%)...</div>;
  }
};

export default HFieldGraph;
