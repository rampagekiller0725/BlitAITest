...
import { selectGeneratedMeshYZ } from "state/reducers/generatedMeshSlice";
...
export interface EditYZProps {
  setVisible: (value: boolean) => void;
  visible: boolean;
}

function EditYZMenu({ visible, setVisible }: EditYZProps) {
  ...
  const generatedMeshYZ = useAppSelector(selectGeneratedMeshYZ);
  let currentMesh = useSelector(selectGeneratedMesh);

  document.addEventListener("keydown", (event) => {
    if(visible) {
      if(event.key == 'Escape') 
        document.getElementById('edityz-btn-cancel')?.click();
      else if(event.key == "Enter")
        document.getElementById('edityz-btn-ok')?.click();
    }
  });

  useEffect(() => {
    if (visible) {
      if (generatedMeshYZ.length != 0) setYZ(generatedMeshYZ.toString());
    }
  }, [visible]);
  ...
  const applyMesh = (isCancel: boolean, previewOnly: boolean = false) => {
    if (!isCancel && getArray(YZ).length < 20) {
    ...
    if (arrayModel.length > 0) {
      ...
      const x = isCancel ? currentMesh.x : getArray(YZ);
      ...
      <form>
        <div className="mt-4 grid grid-cols-20 gap-x-6 gap-y-4">
          <div className="col-span-full">
            ...
            >
              Comma-separated list of mesh lines for the YZ plane (x-axis):
            </label>
